...
const Button = ({
  ...
  const generateClassNames = () => {
    ...
    switch (disabled || submitted) {
      ...
      default:
        switch (variant) {
          case "primary":
            classNames = "bg-red3 text-white hover:bg-red2 hover:text-white";
            break;
  ...
  const checkState = () => {
    ...
    switch (submitted) {
      case true:
        finalChildren = <div>Submitted!</div>;
        break;
        ...
        switch (loading) {
          case true:
            finalChildren = finalChildren = (
              <svg
                aria-hidden="true"
                class="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-white fill-gold"
                viewBox="0 0 100 101"
                fill="none"
  ...
  return (
    <div>
      ...
      {submitted ? (
        <p className="mt-5">
          {`Thank you again for reaching out! I'll get back to you as soon as
          possible :)`}
        </p>
      ) : null}
    </div>
